...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      const [filesCountRes, tasksCountRes, filesRes, tasksRes] = await Promise.all([
        supabase.from('files').select('*', { count: 'exact', head: true }),
        supabase.from('tasks').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
        supabase.from('files').select('id, name, created_at, mime_type, channel_id').order('created_at', { ascending: false }).limit(5),
      ...
      setTasksCount(tasksCountRes.count || 0)
      setRecentFiles((filesRes.data as FileData[]) || [])
      setRecentTasks((tasksRes.data as TaskData[]) || [])
      setLoading(false)
    }
    fetchDashboardData()
  }, [])
          ...
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {profile?.organizations?.name} • {roleLabels[profile?.role || ''] || profile?.role}
              </p>
            </div>
          </div>
          ...
          <>
            ...
            <div className="flex items-center gap-6 mb-8 text-sm">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-lg font-bold text-foreground leading-tight">{filesCount}</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-lg font-bold text-foreground leading-tight">{tasksCount}</p>
                  ...
                  {recentFiles.length > 0 ? (
                    <ul className="space-y-3">
                      {recentFiles.map((file) => (
                        <li key={file.id}>
                          <Link
                            to={file.channel_id ? `/files?channel=${file.channel_id}` : '/files'}
