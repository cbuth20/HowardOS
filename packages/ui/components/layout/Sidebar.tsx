...
export function Sidebar({ userRole = 'client', orgName, userName, userEmail, userAvatar, userOrgs = [], navItems, appLinks = [], isOpen = true, onClose }: SidebarProps) {
  ...
  const initialExpanded = filteredNavItems.find(item => {
  ...
  const [expandedNav, setExpandedNav] = useState<string | null>(
    initialExpanded?.label || null
  )

  // Keep expanded state in sync with pathname
  useEffect(() => {
    const matchingItem = filteredNavItems.find(item => {
      if (!item.children) return false
      const basePath = item.href.split('/').slice(0, 2).join('/')
      return pathname?.startsWith(basePath)
    })
    if (matchingItem) {
      setExpandedNav(matchingItem.label)
    }
  }, [pathname])
      ...
      <div className="p-4">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className="w-full flex items-center gap-3 p-3 bg-primary rounded-lg shadow-sm hover:shadow-md hover:opacity-90 transition-all"
            >
              <HowardAvatar
